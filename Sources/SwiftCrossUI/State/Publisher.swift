    ...
    public class Publisher {
        /// The underlying OpenCombine subject
        ...
        public func observe(with closure: @escaping () -> Void) -> Cancellable {
            let cancellable = subject.sink(receiveValue: closure)
            return Cancellable {
                cancellable.cancel()
            }
            .tag(with: tag)
        ...
        /// Links the publisher to an upstream publisher.
        public func link(toUpstream publisher: Publisher) -> Cancellable {
            let cancellable = publisher.observe {
                self.send()
            }
        ...
        /// A simplified version of `observeAsUIUpdater` for WASI.
        /// Since there's no real main thread or throttling, it just observes and calls the action.
    ...
    public class Publisher {
        ...
        /// with our lives.
        nonisolated private class UpdateStatistics: @unchecked Sendable {
            ...
            return observe {
                ...
                serialUpdateHandlingQueue.async {
                    ...
                    if ProcessInfo.processInfo.systemUptime
                        - updateStatistics.lastUpdateMergeTime
                        < 1
                    {
